...
_NAME_END = 4 + 8 + len(b"StructureSensor")
_SIZES_AT = _NAME_END + 4 * 64 + 8
_FRAMES_AT = _SIZES_AT + 16 + 4


def _patch(path, offset, fmt, value):
    data = bytearray(path.read_bytes())
    data[offset:offset + struct.calcsize(fmt)] = struct.pack(fmt, value)
    path.write_bytes(bytes(data))


@pytest.mark.parametrize("corruption", ["version", "name_length", "depth_width", "frame_count",
                                        "truncated_payload", "compression"])
def test_sens_header_is_validated_against_the_file(tmp_path, corruption):
    scene = _scene(tmp_path)
    path = scene / "scene0010_00.sens"
    if corruption == "version":
        _patch(path, 0, "<I", 3)
    elif corruption == "name_length":
        _patch(path, 4, "<Q", 2**40)
    elif corruption == "depth_width":
        _patch(path, _SIZES_AT + 8, "<I", 0)
    elif corruption == "frame_count":
        _patch(path, _FRAMES_AT, "<Q", 10**12)
    elif corruption == "compression":
        _patch(path, _SIZES_AT - 8, "<i", 9)
    else:
...
def test_sens_depth_that_decodes_to_the_wrong_size_is_rejected(tmp_path):
    scene = _scene(tmp_path)
    path = scene / "scene0010_00.sens"
    # Claim a larger depth image than the payload inflates to.
    _patch(path, _SIZES_AT + 8, "<I", 32)
    seq = sn.ScanNetSensSequence(path)
