...
def test_image_decode_makes_one_owned_copy_and_handles_big_endian(monkeypatch):
    rgb = (np.arange(4 * 5 * 3) % 251).astype(np.uint8).reshape(4, 5, 3)
    msg = ros_msgs.numpy_to_image(rgb, "rgb8")
    decoded = ros_msgs.image_to_numpy(msg)
    assert decoded.flags.c_contiguous and decoded.flags.owndata and decoded.flags.writeable
    decoded[0, 0] = 0  # mutating the result never touches the message
    assert np.array_equal(ros_msgs.image_to_numpy(msg), rgb)
    calls = []
    real_bytes = bytes
    monkeypatch.setattr("builtins.bytes", lambda *a: calls.append(a) or real_bytes(*a))
    ros_msgs.image_to_numpy(msg)
    assert not calls  # no bytes(msg.data) staging copy

    depth = np.array([[1, 2, 3], [4, 5, 65535]], dtype=np.uint16)
    big = sensor_msgs.Image(height=2, width=3, encoding="16UC1", is_bigendian=True, step=6,
                            data=depth.astype(">u2").tobytes())
    out = ros_msgs.image_to_numpy(big)
    assert out.dtype == np.uint16 and out.dtype.isnative and np.array_equal(out, depth)


def test_short_image_data_is_rejected():
    msg = ros_msgs.numpy_to_image(np.zeros((2, 3, 3), np.uint8), "rgb8")
    msg.data = bytes(10)
    with pytest.raises(ValueError):
        ros_msgs.image_to_numpy(msg)


def _info(d=(0., 0., 0., 0., 0.)):
    info = sensor_msgs.CameraInfo(width=640, height=480, d=list(d))
    info.k = [500., 0., 320., 0., 500., 240., 0., 0., 1.]
    info.p = [480., 0., 310., 0., 0., 470., 235., 0., 0., 0., 1., 0.]
    return info


def test_rectified_streams_use_projection_matrix_and_distortion_is_explicit():
    info = _info(d=(-0.2, 0.05, 0., 0., 0.))
    raw = ros_msgs.camera_info_to_intrinsics(info)
    assert (raw.fx, raw.cx) == (500., 320.)  # historical K-based behaviour kept by default
    rect = ros_msgs.camera_info_to_intrinsics(info, rectified=True)
    assert (rect.fx, rect.fy, rect.cx, rect.cy) == (480., 470., 310., 235.)
    assert ros_msgs.camera_info_has_distortion(info)
    with pytest.raises(ValueError, match="distortion"):
        ros_msgs.camera_info_to_intrinsics(info, allow_distortion=False)
    assert ros_msgs.camera_info_to_intrinsics(_info(), allow_distortion=False).fx == 500.
