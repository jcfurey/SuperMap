...
def test_confirm_tentative_discards_low_confidence_after_enough_observations():
    ...
    obj.label_belief = {"chair": 0.5, "stool": 0.3, "bench": 0.2}  # never settled on a label
    obj.hits = 5
...
def test_compact_body_retires_when_only_a_dense_floor_fragment_is_supported():
    ...
    m.update_unmatched(obj, K, T, depth)
    assert obj.status != ObjectStatus.DISAPPEARED  # one contradicting frame is not enough (review C3)
    m.update_unmatched(obj, K, T, depth)
    assert obj.status == ObjectStatus.DISAPPEARED
