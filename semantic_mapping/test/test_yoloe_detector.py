...
class _Tensor:
    ...
    def cpu(self):
        type(self).transfers.append(self.data.dtype)
        return self
...
def test_yoloe_transfers_once_thresholds_on_device_and_uses_half_on_cuda(monkeypatch):
    captured = {}

    class _PackedBoxes:
        data = _Tensor([[200.0, 100.0, 240.0, 140.0, 0.9, 0.0], [10.0, 10.0, 20.0, 20.0, 0.6, 1.0]])
        ...
        @property
        def xyxy(self):
            raise AssertionError("per-field transfers are not needed when boxes.data is available")

    def predict(image, **kwargs):
        captured.update(kwargs)
        masks = np.zeros((2, *image.shape[:2]), dtype=np.float32)
        masks[0, 100:140, 200:240] = 0.9
        masks[1, 10:20, 10:20] = 0.4  # below the mask threshold
        return [SimpleNamespace(boxes=_PackedBoxes(), names={0: 'chair', 1: 'table'},
    ...
    assert [d.label for d in detections] == ['chair', 'table']
    assert detections[0].mask.sum() == 1600 and not detections[1].mask.any()
    assert detections[1].score == pytest.approx(0.6)
    assert _Tensor.transfers == [np.float64, np.bool_]  # one box table, one bool mask stack
    captured.clear()
