...
class BackgroundExecutor:
    """Spin nodes on a MultiThreadedExecutor in a thread, as ``run_node`` does in production."""
...
def wait_for(predicate, timeout: float = 5.0) -> None:
    """Poll while another thread spins."""
    deadline = time.monotonic() + timeout
    while not predicate() and time.monotonic() < deadline:
        time.sleep(0.02)
    assert predicate(), 'condition not reached before the deadline'
