...
def node_factory(dataset):
    ...
    ``rclpy.init``. The node is configured and activated (lifecycle autostart)
    unless ``activate=False``. Publishers are silenced unless
    ...
    def make(*ros_args: str, silence_publishers: bool = True, activate: bool = True):
        if rclpy.ok():
            for node in nodes:
                node.destroy_node()
            nodes.clear()
        ...
        if not activate:
            return node
        node.autostart_now()  # configure + activate without spinning, as the autostart timer would
        assert node._active, "node failed to configure/activate"
        if silence_publishers:
            for publisher in (node.obj_boxes_pub, node.obj_points_pub, node.objects_pub, node.annotated_image_pub):
