...
nearest depth source within ``--sync_slop`` seconds; one depth frame may
serve several RGB frames. A point cloud captured at ``t_cloud`` is moved into
the camera at the image time ``t_rgb`` through the world frame
(``lookup_transform_full``), so sensor motion inside the sync window is
compensated rather than ignored.

The output uses a distortion-free pinhole model. A raw camera topic whose
CameraInfo has nonzero ``D`` is rejected; pass ``--rectified`` for an
``image_rect*`` topic, whose projection matrix ``P`` is then used instead of
``K``.
...
def lookup_se3_full(buffer, target_frame: str, target_s: float, source_frame: str, source_s: float,
                    fixed_frame: str) -> np.ndarray:
    """``T_target(t_target) <- source(t_source)``, chained through a frame fixed in time (the world)."""
    from rclpy.time import Time

    return transform_to_se3(buffer.lookup_transform_full_core(
        target_frame, Time(seconds=target_s), source_frame, Time(seconds=source_s), fixed_frame))


def calibration_from_camera_info(info, rectified: bool):
    """Pinhole intrinsics for the output, refusing distortion the format cannot carry."""
    if not rectified and camera_info_has_distortion(info):
        raise ValueError(
            f"CameraInfo has nonzero distortion D={list(info.d)}; the sequence format is an undistorted "
            "pinhole model. Convert a rectified image topic with --rectified (uses P), or undistort first.")
    return camera_info_to_intrinsics(info, rectified=rectified)
...
def main() -> None:
    ...
    # Several RGB frames can pair with one depth frame: keep it until its last consumer is written.
    depth_refs = Counter(pairs.values())
    ...
    def emit(rgb_stamp: float, rgb: np.ndarray) -> None:
        nonlocal frame_id
        depth_stamp = pairs[rgb_stamp]
        depth_source, source_frame, source_stamp = depth_cache[depth_stamp]
        depth_refs[depth_stamp] -= 1
        if depth_refs[depth_stamp] <= 0:
            del depth_cache[depth_stamp]
        ...
        try:
            ...
            if args.depth_topic:
                ...
                # Cloud points at the cloud's capture time, camera at the image's capture time.
    ...
    for topic, msg, t_ns in reader.messages([args.rgb_topic, args.camera_info_topic, depth_topic]):
        ...
        if topic == args.camera_info_topic:
            if intrinsics is None:
                try:
                    intrinsics = calibration_from_camera_info(msg, args.rectified)
                except ValueError as exc:
                    raise SystemExit(str(exc)) from exc
                (out_dir / "intrinsics.json").write_text(json.dumps(intrinsics.__dict__, indent=2))
            continue
        if topic == depth_topic:
            if depth_refs.get(stamp, 0) > 0 and stamp not in depth_cache:
        ...
        if intrinsics is None:
            continue  # hold frames until the first CameraInfo (it may trail the image by a few ms)
