...
second update, which would square the likelihood ratio."""
...
def new_belief(label: str, score: float, prior: float = 1.0) -> LabelBelief:
    ...
    The belief is a normalized distribution from the start -- the only label
    under consideration holds all the mass -- so ``label_confidence`` means
    the same thing before and after the first Eq. (10) update. The detector
    score enters every later update as the strength of the likelihood.
    ``score`` and ``prior`` are kept for API compatibility.
    """
    del score, prior
    return {label: 1.0}
