...
def project_and_classify(
    ...
    ``contradiction_window_px`` (radius, 0 = off) makes the DISAPPEARED
    verdict conservative at silhouettes: a point is only "seen through" when
    the sensor surface lies behind it at every valid pixel of the
    (2r+1)^2 window around its projection, i.e. against the *nearest*
    reading nearby. A point on an object's outline whose rounded pixel
    lands on the background, while its neighbours still see the object, is
    left without evidence instead of being contradicted, so edges no longer
    erode one rounding error at a time.
    ...
    radius = int(contradiction_window_px)
    if radius > 0 and np.any(disappeared):
        idx = np.flatnonzero(disappeared)
        nearest = sensor_depth[idx].copy()
        for dv in range(-radius, radius + 1):
            for du in range(-radius, radius + 1):
                if du == 0 and dv == 0:
                    continue
                u = np.clip(us[idx] + du, 0, w - 1)
                v = np.clip(vs[idx] + dv, 0, h - 1)
                reading = depth_image[v, u]
                ok = np.isfinite(reading) & (reading > 0)
                nearest = np.where(ok, np.minimum(nearest, reading), nearest)
        disappeared[idx] = d_proj[idx] - nearest < -tau_eps
...
def min_contradiction_prune_threshold(prune_threshold: float, min_contradictions: int,
                                     p_miss: float = 0.1) -> float:
    """Prune threshold such that a fresh point (log-odds 0) needs at least
    ``min_contradictions`` contradicting frames before it is pruned.

    Each contradiction adds ``logit(p_miss)`` (-2.2 at the default 0.1), so a
    threshold of -1.5 let a single glitched frame -- a pose jump, a depth
    dropout at a silhouette -- delete a new point for good. The configured
    threshold is lowered to half a step past ``min_contradictions - 1``
    contradictions; it is never raised. ``min_contradictions <= 1`` returns
    the configured threshold unchanged.
    """
    if min_contradictions <= 1:
        return float(prune_threshold)
    step = float(logit(p_miss))
    return float(min(prune_threshold, (min_contradictions - 0.5) * step))
