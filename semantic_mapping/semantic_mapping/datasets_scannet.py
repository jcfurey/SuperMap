...
Known approximation (review item C36): ScanNet's colour and depth cameras have
different intrinsics (``intrinsic_color`` vs ``intrinsic_depth``) and a small
colour-to-depth extrinsic. Resizing colour to the depth grid and projecting
everything with the depth ``K`` ignores both, so colour pixels (and therefore
2D detections and masks) land a few depth pixels away from the geometry they
image, mostly near object boundaries. This matches how most ScanNet
benchmarks consume the data; a registration step (warp colour through
``intrinsic_color``/``extrinsic_color`` into the depth frame) would remove it. Poses that ScanNet marks invalid (``-inf`` entries) are
...
def _resize_rgb(rgb: np.ndarray, width: int, height: int) -> np.ndarray:
    """Resample colour onto the depth grid (a plain resize, not a registration; see module notes)."""
...
class ScanNetSensSequence:
    ...
    used, as with the exported layout (a few pixels of colour/depth
    misalignment; see the module notes).

    The header is validated before anything is allocated: only version 4 (the
    format ScanNet ships) is accepted, sizes must be sane, and the frame count
    and every record length must fit in the file, so a corrupt or hostile file
    fails with ``ValueError`` instead of a huge allocation or a garbage decode.
    """

    HEADER_FLOATS = 16
    SUPPORTED_VERSION = 4
    MAX_SENSOR_NAME_BYTES = 4096
    MAX_IMAGE_SIDE = 16384
    FRAME_HEAD_BYTES = 64 + 8 + 8 + 8 + 8
    ...
    def _read_exact(self, f, size: int, what: str) -> bytes:
        data = f.read(size)
        if len(data) != size:
            raise ValueError(f"{self.sens_path}: truncated while reading {what}")
        return data

    def _read_header(self, f) -> None:
        self._file_size = self.sens_path.stat().st_size
        (self.version,) = struct.unpack("<I", self._read_exact(f, 4, "version"))
        if self.version != self.SUPPORTED_VERSION:
            raise ValueError(f"{self.sens_path}: unsupported .sens version {self.version} "
                             f"(expected {self.SUPPORTED_VERSION})")
        (name_length,) = struct.unpack("<Q", self._read_exact(f, 8, "sensor name length"))
        if name_length > self.MAX_SENSOR_NAME_BYTES:
            raise ValueError(f"{self.sens_path}: implausible sensor name length {name_length}")
        self.sensor_name = self._read_exact(f, name_length, "sensor name").decode("utf-8", errors="replace")
        matrices = []
        for _ in range(4):
            matrices.append(np.array(struct.unpack("<16f", self._read_exact(f, 64, "calibration")),
                                     dtype=np.float64).reshape(4, 4))
        self.intrinsic_color, self.extrinsic_color, self.intrinsic_depth, self.extrinsic_depth = matrices
        color_compression, depth_compression = struct.unpack("<ii", self._read_exact(f, 8, "compression"))
        if color_compression not in SENS_COLOR_COMPRESSION:
            raise ValueError(f"{self.sens_path}: unknown colour compression {color_compression}")
        self.color_compression = SENS_COLOR_COMPRESSION[color_compression]
        self.depth_compression = SENS_DEPTH_COMPRESSION.get(depth_compression, str(depth_compression))
        self.color_width, self.color_height, self.depth_width, self.depth_height = struct.unpack(
            "<4I", self._read_exact(f, 16, "image sizes"))
        for name in ("color_width", "color_height", "depth_width", "depth_height"):
            if not 0 < getattr(self, name) <= self.MAX_IMAGE_SIDE:
                raise ValueError(f"{self.sens_path}: implausible {name} {getattr(self, name)}")
        (self.depth_shift,) = struct.unpack("<f", self._read_exact(f, 4, "depth shift"))
        if not np.isfinite(self.depth_shift) or self.depth_shift < 0:
            raise ValueError(f"{self.sens_path}: invalid depth shift {self.depth_shift}")
        (self.num_frames,) = struct.unpack("<Q", self._read_exact(f, 8, "frame count"))
        ...
        if not (np.isfinite(k).all() and k[0, 0] > 0 and k[1, 1] > 0):
            raise ValueError(f"{self.sens_path}: invalid depth intrinsics")
        remaining = self._file_size - f.tell()
        if self.num_frames * self.FRAME_HEAD_BYTES > remaining:
            raise ValueError(f"{self.sens_path}: header claims {self.num_frames} frames but only "
                             f"{remaining} bytes follow")
    ...
    def _index_frames(self, f) -> None:
        ...
        raw_depth_bytes = self.depth_width * self.depth_height * 2
        for i in range(self.num_frames):
            head = f.read(self.FRAME_HEAD_BYTES)
            if len(head) < self.FRAME_HEAD_BYTES:
            ...
            offset = f.tell()
            if color_bytes + depth_bytes > self._file_size - offset:
                raise ValueError(f"{self.sens_path}: frame {i} payload runs past the end of the file")
            if self.depth_compression == "raw_ushort" and depth_bytes != raw_depth_bytes:
                raise ValueError(f"{self.sens_path}: frame {i} raw depth has {depth_bytes} bytes, "
                                 f"expected {raw_depth_bytes}")
            self._stamps[i] = (stamp_depth or stamp_color) * 1e-6 if (stamp_depth or stamp_color) else i / SCANNET_FPS
            self._records.append((offset, color_bytes, depth_bytes))
    ...
    def _decode(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        expected = self.depth_width * self.depth_height * 2
        try:
            if self.depth_compression == "zlib_ushort":
                # Bounded decompression: never inflate more than one depth image.
                inflater = zlib.decompressobj()
                raw = inflater.decompress(depth_data, expected + 1)
            else:
                raw = depth_data
        except zlib.error as exc:
            raise ValueError(f"{self.sens_path}: corrupt depth frame {index}: {exc}") from exc
        if len(raw) != expected:
            raise ValueError(f"{self.sens_path}: depth frame {index} decodes to {len(raw)} bytes, expected {expected}")
