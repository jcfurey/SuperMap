...
class ObjectStats:
    ...
    matched_id_frames: dict[int, int] = field(default_factory=dict)
    """Per instance ID, the number of appearance frames in which it matched."""
...
class SequenceEvaluator:
    ...
    def identity_consistency(self) -> dict:
        ...
        "Served" means the ID that matched the phase in the most frames (its
        majority ID; ties admit every tied ID): an identity is consistent when
        one ID is the majority in every phase. An ID that merely appeared in
        each phase once, while fragments carried most frames, does not count.
        """
        phases: dict[str, list[set[int]]] = {}
        for gt, stats in zip(self.ground_truth, self.stats):
            counts = stats.matched_id_frames
            top = max(counts.values(), default=0)
            phases.setdefault(gt.identity, []).append({i for i, n in counts.items() if n == top and n > 0})
