...
- **D1 fixed.**
  ...
  - Follow-up: under 20% label flicker (the paper harness's stress scene), flipped detections still started co-located instances. The relabelling stage compared a detection only with the track's predicted box. That prediction keeps the last box size, so it overstates a partly occluded or cut-off object, and its motion gate rejected size jumps. A second image test now compares the detection with the part of the instance its points show in the current depth (`ObjectMap.visible_bbox`), with no motion gate since it is measured this frame. A small object in front of a fully visible one still starts its own instance. Over 5 seeds, final-map precision with flicker alone rises from 0.62 to 0.81 (0.89 without flicker). On the harness's stress scene, which adds depth dropout, it rises from 0.54 to 0.65 and F1 from 0.70 to 0.79. Maps of scenes without flicker are unchanged.
  - Second follow-up: the rest of the stress scene's duplicates and wrong labels.
    - Detections without depth on the object could join an instance only through the tracker's prediction, which a partly hidden object's detection misses. A new label-gated stage matches them against the part of the instance the depth does not rule out (`association.associate_unlifted`; `ObjectMap.visible_bbox` now counts points on pixels without a reading). Under a label the instance has not taken, they also need its appearance (`reid_min_similarity`), since without depth nothing tells the same object from a person standing in front of it.
    - Slivers a few pixels wide, at the image border or past an occluder, cannot be compared in 2D. Their duplicates are merged afterwards (`merge_alternating_min_overlap`): two co-located instances that take turns being detected, never in the same frame, are one object whose label flickers. Two objects in one place are detected together, and a replacement is a handover, so neither merges. Instances record their recent match stamps for this (`match_stamps`, persisted).
    - With the flipped detections now reaching the object, Eq. 10's new-label prior let three flips in a row relabel it, however long it had been seen as something else: at 20% flicker, about one object in four over 60 frames. The prior (and pruning floor) is now 1e-4, so an unseen label needs four consistent observations (`semantic_fusion.NEW_LABEL_PRIOR`).
    - Final-map precision is 0.89 with flicker, with flicker and dropout, and with dropout alone, the same as without either; recall stays 1.0. Harness stress-scene F1 is 0.94. Maps of the default configuration without flicker or dropout are unchanged; sparse-depth configurations create fewer spurious IDs.
- **D3 fixed.**
